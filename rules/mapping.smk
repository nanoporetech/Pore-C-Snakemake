...
rule haplotag:
    ...
    log: to_log(paths.mapping.haplotagged_aligns)
    benchmark: to_benchmark(paths.mapping.haplotagged_aligns)
    wrapper: "file:../wrappers/whatshap/haplotag"
...
rule merge_contact_files:
    output: directory(paths.merged_contacts.contacts)
    input: expand_basecall_batches(paths.contacts.contacts)
...
rule summarise_contacts:
    output: paths.merged_contacts.concatemers
    input: paths.merged_contacts.contacts
