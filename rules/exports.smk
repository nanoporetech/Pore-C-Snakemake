...
rule to_haplotyped_cooler:
    ...
    log: to_log(paths.matrix.haplotyped_cools)
    benchmark: to_benchmark(paths.matrix.haplotyped_cools)
    threads: config['software']['pore_c']['to_cooler']['threads']
    conda: PORE_C_CONDA_FILE
    shell:
        "pore_c {DASK_SETTINGS} --dask-num-workers {threads} "
...
rule create_mcool_file:
    ...
    benchmark: to_benchmark(paths.matrix.mcool)
    log: to_log(paths.matrix.mcool)
...
rule to_unsorted_pairs:
    ...
    log: to_log(paths.pairs.unsorted_pairs)
    benchmark: to_benchmark(paths.pairs.unsorted_pairs)
...
rule to_sorted_pairs:
    ...
    input: paths.pairs.unsorted_pairs
    params:
        uncomp_file = to_prefix(paths.pairs.pairs, 1)
    log: to_log(paths.pairs.pairs)
    benchmark: to_benchmark(paths.pairs.pairs)
...
rule index_pairs:
    output: paths.pairs.index
    input: paths.pairs.pairs
    log: to_log(paths.pairs.index)
    benchmark: to_benchmark(paths.pairs.index)
    conda: "../envs/pairix.yml"
    shell:
        "pairix {input} 2>{log}"
...
rule download_juicer_tools:
    output: paths.juicebox.tools
    params:
        url = config['software']['juicer']['tools_url']
    conda: "../envs/juicer_tools.yml"
    log: to_log(paths.juicebox.tools)
    shell:
...
rule create_hicRef:
    ...
    input: paths.virtual_digest.fragments 
